...
export interface ActivePassiveClientConfig extends Omit<
...
export class ActivePassiveCloudTowerClient extends CloudTowerClient {
  ...
  constructor(user: CloudTowerUser, config: ActivePassiveClientConfig) {
    ...
    super(user, {
      ...
      baseURL: {
        rootURL: "",
        apiBasePath: basePath ?? DEFAULT_API_BASE_PATH,
      },
      probePath,
    });
  ...
  private async probeEndpoint(
    ...
    const response = await this.instance.request({
      method: "GET",
      url: this.probePath,
      baseURL: endpoint.endpointBaseURL,
      maxRedirects: 0,
      timeout: endpoint.probeTimeout,
...
function createEndpoints(
  ...
  for (const rawEndpoint of endpoints) {
    const endpoint = createEndpoint(rawEndpoint, basePath, probeTimeout);
...
function createEndpoint(
  ...
  const endpointBaseURL = joinURL(url.origin, url.pathname);
  const apiBaseURL = joinURL(endpointBaseURL, basePath);
