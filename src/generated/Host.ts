...
export class HostApi<SecurityDataType = unknown> {
   ...
   * @secure
   * @response `200` `{ shutdownVms: ({ vm_uuid: string })[], done: boolean }`
   * @response `400` `ErrorBody` Bad request
   * @response `404` `ErrorBody` Not found
   * @response `500` `ErrorBody` Server error
   */
  enterMaintenanceModePrecheckResult = (
    ...
    this.http.request<
      { shutdownVms: { vm_uuid: string }[]; done: boolean },
      ErrorBody
    >({
   ...
   * @secure
   * @response `200` `{ offlineMigrateVms: (MaintenanceModeVmInfo)[], liveMigrateVms: (MaintenanceModeVmInfo)[], shutDownVms: (MaintenanceModeVmInfo)[] }`
   * @response `400` `ErrorBody` Bad request
   * @response `404` `ErrorBody` Not found
   * @response `500` `ErrorBody` Server error
   */
