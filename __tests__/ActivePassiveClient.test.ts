...
} from "../src/generated/ActivePassiveClient";
...
describe("CloudTowerClient.probeActivePassive", () => {
  ...
  it("uses the default API base path only as a configurable probe boundary", async () => {
    const first = createSingleEndpointClient(
      `${endpoints.first}/v2/api`,
      service,
    );
  ...
  it("splits a string baseURL into an origin root and API base path", async () => {
    const probe = vi.fn();
    const client = new CloudTowerClient(
      ...
      {
        baseURL: `${endpoints.second}/gateway/custom-api`,
        probePath: "/healthz",
        adapter: async (config) => {
          const request = parseRequest(config);
          expect(request.origin).toBe(endpoints.second);
          expect(request.path).toBe("/healthz");
          probe();
          return settle(config, { status: 200, data: "" });
        },
      },
    );

    await expect(client.probeActivePassive()).resolves.toBe(true);

    expect(probe).toHaveBeenCalledOnce();
  });

  it("accepts an object baseURL with separate root URL and API base path", async () => {
    ...
    const client = new CloudTowerClient(
      ...
      {
        baseURL: {
          rootURL: `${endpoints.second}/gateway`,
          apiBasePath: "/custom-api",
        },
        probePath: "/healthz",
        adapter: async (config) => {
          const request = parseRequest(config);
          requests.push(`${request.origin}${request.path}`);

          if (request.path === "/gateway/healthz") {
            return settle(config, { status: 200, data: "" });
          }
          if (request.path === "/gateway/custom-api/login") {
            return settle(config, {
              status: 200,
              data: { data: { token: "token-second" } },
            });
          }
          if (request.path === "/gateway/custom-api/get-vms") {
          ...
          throw new Error(
            `unexpected object baseURL request: ${request.origin}${request.path}`,
          );
        },
      },
    );
    ...
    expect(requests).toEqual([
      `${endpoints.second}/gateway/healthz`,
      `${endpoints.second}/gateway/custom-api/login`,
      `${endpoints.second}/gateway/custom-api/get-vms`,
    ]);
  });
});
...
describe("ActivePassiveClient", () => {
  ...
  function useClient(
    failoverStrategy = ActivePassiveFailoverStrategy.AutoFailover,
  ): void {
  ...
  describe("when endpoints include gateway paths", () => {
    it("keeps endpoint paths for gateway-routed probes and API calls", async () => {
      const requests: string[] = [];
      const gatewayEndpoints = {
        first: `${endpoints.first}/tower-first`,
        second: `${endpoints.second}/tower-second`,
      };
      const gatewayClient = new ActivePassiveClient(
        {
          username: "root",
          password: "password",
          source: UserSource.LOCAL,
        ...
        {
          endpoints: [gatewayEndpoints.first, gatewayEndpoints.second],
          basePath: "/custom-api",
          probePath: "/healthz",
          adapter: async (config) => {
            const request = parseRequest(config);
            ...
            if (request.path === "/tower-first/healthz") {
              return settle(config, { status: 307, data: "" });
            }
            if (request.path === "/tower-second/healthz") {
              return settle(config, { status: 200, data: "" });
            }
            if (request.path === "/tower-second/custom-api/login") {
              return settle(config, {
                status: 200,
                data: { data: { token: "token-second" } },
              });
            }
            if (request.path === "/tower-second/custom-api/get-vms") {
              expect(config.headers?.Authorization).toBe("token-second");
              return settle(config, {
                status: 200,
                data: [{ id: "vm-second-1" }],
            ...
            throw new Error(
              `unexpected gateway request: ${request.origin}${request.path}`,
            );
          },
        },
      );
      const gatewayVmApi = new VmApi(gatewayClient);

      const response = await gatewayVmApi.getVms({ first: 10 });

      expect(response.data).toEqual([{ id: "vm-second-1" }]);
      expect(gatewayClient.currentActiveEndpoint()).toBe(
        gatewayEndpoints.second,
      );
      expect(requests).toEqual([
        `${endpoints.first}/tower-first/healthz`,
        `${endpoints.second}/tower-second/healthz`,
        `${endpoints.second}/tower-second/custom-api/login`,
        `${endpoints.second}/tower-second/custom-api/get-vms`,
      ]);
    });
  });
